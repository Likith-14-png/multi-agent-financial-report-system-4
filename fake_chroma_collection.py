...
=====================
A minimal stand-in for a ChromaDB `Collection` object, exposing the same
`.query()` / `.get()` shapes ResearchAgent relies on. Used ONLY so
ResearchAgent's control flow (decomposition, retrieval, citation-building,
synthesis) can be exercised in environments without network access to
install chromadb / sentence-transformers or download an embedding model.

Retrieval here is keyword-overlap scoring (not real embeddings) — good
enough to prove the plumbing is correct. Swap in a real chromadb Collection
(see demo_index.py) for actual semantic retrieval.
"""
import re
import uuid
    ...
    def add_document(self, path: str, company: str, doc_type: str, period: str = ""):
        ...
        for idx, chunk in enumerate(_chunk_text(text)):
            ...
            self._metas.append({
                ...
                "section": _guess_section(chunk), "source_file": Path(path).name,
                "period": period,
            })

    # ---- chromadb-compatible interface ----
    def query(self, query_texts: List[str], n_results: int = 4,
              where: Optional[Dict] = None) -> Dict[str, List[List[Any]]]:
